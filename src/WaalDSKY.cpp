...
void actionSetDate()
{
    byte yearToSet[4] = {2, 0, 2, 4}; // Example: 2024
    byte monthToSet[2] = {0, 1};      // Example: January
    byte dayToSet[2] = {0, 1};        // Example: 1st

    // TODO: Replace the above with actual grabbing of values from the DSKY.
    ...
    realTimeClock.adjust(DateTime(
                                  ((yearToSet[0] * 1000) + (yearToSet[1] * 100) + (yearToSet[2] * 10) + yearToSet[3]),
...
{
      if (toggle == true) //so every other second
    {
        if ((toggle250 == true) && (toggled250 == false))
        {
            setLamp(white_dim, lampClk);
            toggled250 = true;
        }
...
void setup()
{
    ...
    // Toggle 
    timer.every(1000, toggle_timer); //every second, call toggle_timer
    timer.every(600, toggle_timer_600); // every 600ms, call toggle_timer_600
    timer.every(250, toggle_timer_250); // every 250ms, call toggle_timer_250
  ...
  startUp(); 
  while (!Serial)
    ; //in case we're connected via USB, wait until Serial becomes available;
  delay(100);
  clearRegister(1);
  delay(100);
  clearRegister(2);
